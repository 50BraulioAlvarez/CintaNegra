...
module.exports = {
    ...
    login: async (req, res) => {
        try {
            ...
            const user = await userServices.finById(req.body.email);
            if (!user) res.status(404), send({ message: 'Usuario no encontrado' });

            //Comparo las contraseñas
            const isMatch = user.Services.comparePasswords(req.body.password, user, password);
            ...
            //Creo el token a partir del payload
